...
Source files of a language based on the \ref{gloss:dotnetlabel} platform can have structured comments that produce documentation for types and members defined in said source files \cite{wagner_xml_2022}. The respective language compiler (C\# and VB are compiled by Roslyn, while F\# has its own specific F\# compiler) produces an \ref{itm:xml} file that contains structured data representing comments bound to types and members \ref{gloss:dotnetlabel}.
...
\begin{lstlisting}[caption=List tag \cite{carr_adding_2012}]
    /// <summary>
